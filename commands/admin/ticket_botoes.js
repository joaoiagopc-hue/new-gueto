    ...
    async handleInteraction(interaction) {
        ...
        if (interaction.customId === 'abrir_ticket_suporte' || interaction.customId === 'abrir_ticket_denuncia') {
            ...
            const canal = await interaction.guild.channels.create({
                ...
                permissionOverwrites: [
                    ...
                    ...(CONFIG.CARGO_STAFF_ID ? [{ id: CONFIG.CARGO_STAFF_ID, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] }] : [])
                ],
            });

            const embed = new EmbedBuilder().setTitle(`🎫 TICKET - ${tipo.toUpperCase()}`).setDescription(`Olá ${interaction.user}.\nRelate seu problema.\n\n🙋‍♂️ **Assumir**\n📜 **Logs**\n🔒 **Fechar**`).setColor('#0000ff').setThumbnail(icone).setTimestamp();
            ...
            await canal.send({ content: `${interaction.user} | ${CONFIG.CARGO_STAFF_ID ? `<@&${CONFIG.CARGO_STAFF_ID}>` : '@Staff'} 🔔`, embeds: [embed], components: [bt] });
            await interaction.editReply({ content: `✅ Canal aberto: ${canal}` });
        }

        if (interaction.customId === 'assumir_ticket') {
            if (CONFIG.CARGO_STAFF_ID && !interaction.member.roles.cache.has(CONFIG.CARGO_STAFF_ID)) return interaction.reply({ content: '❌ Sem permissão!', ephemeral: true });
            if (interaction.message.embeds.description.includes('RESPONSÁVEL ATUAL:')) return interaction.reply({ content: '❌ Já assumido!', ephemeral: true });
            ...
            await interaction.channel.send({ content: `🙋‍♂️ Assumido por ${interaction.user}.` });
        }

        if (interaction.customId === 'salvar_logs_ticket') {
            await interaction.reply({ content: '🔄 Salvando...', ephemeral: true });
            ...
            const cLogs = interaction.guild.channels.cache.get(CONFIG.CANAL_LOGS_TICKETS);
            if (cLogs) await cLogs.send({ content: `📜 Logs de \`#${interaction.channel.name}\``, files: [{ attachment: Buffer.from(txt, 'utf-8'), name: `log-${interaction.channel.name}.txt` }] });
            await interaction.followUp({ content: '✅ Salvo nas logs!', ephemeral: true });
        }

        if (interaction.customId === 'fechar_ticket_tentativa') {
            const msgs = await interaction.channel.messages.fetch({ limit: 50 });
            if (!msgs.some(m => m.content.includes('Assumido por'))) return interaction.reply({ content: '❌ Bloqueado! Nenhum staff assumiu.', ephemeral: true });
            const modal = new ModalBuilder().setCustomId('modal_motivo_fechamento').setTitle('Fechar Atendimento');
            modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('input_motivo_texto').setLabel('Motivo?').setStyle(TextInputStyle.Paragraph).setRequired(true)));
        ...
        if (interaction.isModalSubmit() && interaction.customId === 'modal_motivo_fechamento') {
            ...
            if (cLogs) {
                ...
                await cLogs.send({ embeds: [logEmbed] });
            }

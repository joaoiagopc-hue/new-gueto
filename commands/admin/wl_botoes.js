...
module.exports = {
    ...
    async processarWLAUTOMATICA(interaction) {
        ...
        if (
            ...
            if (interaction.client.wlSessoes.has(interaction.user.id)) {
                return interaction.reply({
                    content: '⚠️ **Exame Ativo:** Você já possui um teste de White-List em andamento neste canal!',
                    ephemeral: true
                });
            }

            interaction.client.wlSessoes.set(interaction.user.id, {
                indiceQuestao: 0,
                acertos: 0
            });

            await interaction.reply({
                content: '📝 **Exame Inicializado!** O bot disparou a primeira questão abaixo. Responda com atenção!',
                ephemeral: true
            });
        ...
        if (
            ...
            const sessaoMembro = interaction.client.wlSessoes.get(
                interaction.user.id
            );

            const partesBotao = interaction.customId.split('_');

            const alternativaEscolhida = partesBotao[2];
            const numeroQuestaoAtual = parseInt(partesBotao[3], 10);

            // Confere se a pergunta clicada é realmente a atual
            if (
                numeroQuestaoAtual !==
                sessaoMembro.indiceQuestao
            ) {
                return interaction.reply({
                    content: '⚠️ **Ação Inválida:** Por favor, responda apenas à pergunta atual.',
                    ephemeral: true
                });
            }
                ...
                if (aprovadoNoTeste) {
                    ...
                    const embedAprovado =
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                `Parabéns ${interaction.user}! O bot avaliou suas respostas e você foi **APROVADO**!\n\n` +
                                `📈 **Resultado Final:** \`${totalAcertosConcluidos} de 7 Acertos\`\n\n` +
                                `🏅 Seus cargos foram atualizados! Seus privilégios civis de morador oficial estão ativos. Seja bem-vindo ao Gueto RP!`
                            )
                            .setColor('#00ff00');
...
function criarEmbedQuestao(questaoExame, index) {
